...
import type { FlowSchema, BlockContent } from '@/types/flow'
...
const sampleSchema: FlowSchema = {
    ...
    blocks: {
        ...
        block_welcome_lang: {
            ...
            group_id: 'group_welcome',
            type: 'button',
            content: { buttons: [{ label: 'Русский' }, { label: 'English' }] },
...
describe('кнопка с callback_data (inline_keyboard): content с ButtonItem[] проходит round-trip', () => {
            ...
            block_lang: {
                ...
                content: {
                    buttons: [
                        ...
                        { label: 'English' }, // без callback_data — допустимо, используется label
                    ],
                },
                config: { keyboardMode: 'inline', variable: 'lang' },
                outgoing_edge_id: null,
            },
        },
        edges: {},
    ...
    it('toVueFlow -> toSchema сохраняет callback_data и кнопки без callback_data без изменений', () => {
        const { nodes, edges } = toVueFlow(buttonSchema)
        const result = toSchema(nodes, edges, buttonSchema.start_group_id)

        expect(result.blocks['block_lang'].content?.buttons).toEqual([
            { label: 'Русский', callbackData: 'lang_ru' },
            { label: 'English' },
        ])
    })
...
describe('обратная совместимость: content.buttons в старом формате string[] (до callback_data)', () => {
    const legacySchema: FlowSchema = {
        start_group_id: 'group_lang',
        groups: {
            group_lang: { id: 'group_lang', title: 'Язык', position: { x: 0, y: 0 }, block_ids: ['block_lang'] },
        },
        blocks: {
            block_lang: {
                ...
                group_id: 'group_lang',
                type: 'button',
                // Старый формат — то, что реально может лежать в БД у уже
                // существующих ботов, заведённых до появления callback_data.
                // Приходится обманывать TS явным приведением типа: сама
                // BlockContent теперь типизирована строго как ButtonItem[],
                // а этот тест намеренно проверяет данные из "прошлого".
                content: { buttons: ['ru', 'en'] } as unknown as BlockContent,
                outgoing_edge_id: null,
            },
        },
        edges: {},
    ...
    it('toVueFlow нормализует string[] в ButtonItem[] на лету', () => {
        const { nodes } = toVueFlow(legacySchema)
        const data = nodes[0].data as GroupNodeData
        expect(data.blocks[0].content?.buttons).toEqual([{ label: 'ru' }, { label: 'en' }])
    })
})
        ...
        const nodesWithButtonVariable = [
            {
                ...
                data: {
                    ...
                    blocks: [
                        ...
                        { id: 'b2', type: 'button', config: { variable: 'user_language' }, content: { buttons: [{ label: 'ru' }, { label: 'en' }] } },
                    ],
                } as GroupNodeData,
            },
        ]
