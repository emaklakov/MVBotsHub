...
class RichTextMarked implements RichTextItem
{
    private const TYPE = 'marked';
     ...
     */
    public static function fromData(string|array $data): RichTextMarked
    {
        $richTextMarked = new self();

        if (!is_array($data) || $data['type'] !== self::TYPE) {
            throw RichTextException::structureMismatch();
        }

        $richTextMarked->text = app(RichTextFactory::class)->fromData($data['text']);
